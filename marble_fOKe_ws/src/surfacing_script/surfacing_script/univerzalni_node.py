...
class UniversalFloaterNode(Node):
    def __init__(self):
        ...
        self.create_timer(10, self.timer_callback) #neka bude manje od minute npr
        self.counter_ = 0
        self.subscriber_ = self.create_subscription(CommsMessage, '/marble_pkg/from_cls_to_comms', self.theme_callback, 10)
        self.publisher_ = self.create_publisher(CommsMessage, '/comms_topic/last_full_frame', 10) #last frame publisher
    ...
    def timer_callback(self):         #ja ovdje rjesavam sve u jednom callbacku
        if  self.counter_ == 3: #ovdje poslije ici do 10
           ...
           if(len(self.message_array) == 3):
             self.message_array[self.counter_ - 1] = self.message_theme_array[self.counter_ - 1] #ovo kad se popuni lista
           else:
               self.message_array.append(self.message_theme_array[self.counter_])
